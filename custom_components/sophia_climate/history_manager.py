...
class ClimateHistoryManager:
    ...
    async def _store_to_rag(self, decision_data: Dict[str, Any]) -> bool:
        ...
        text = (
            f"Climate decision for zone {zone} on {day_str} at {time_str}: {decision}. "
        ...
        # Deterministic doc_id so repeated writes for the same (zone, timestamp) overwrite.
        safe_ts = ts_iso[:19].replace(":", "-").replace(" ", "T")
